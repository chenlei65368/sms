...
var DzmFrame = function(){
	...
	var EasyUI = function(){
		...
		easyui.fmtDatetime= function(val){
			if ($.isEmptyObject(val)){
				return '';
			}
		 ...
		 * 解析日期
		 */
		easyui.parseDate = function(val){
			if (val==''){
				return new Date();
			}
			return new Date(Date.parse(val.replace(/-/g,'/'))).addHours(-14);
		}
	...
	var Uploader = function(){
		var uploader = {};
		uploader.record = function(name, filePath){
			var jsonData = {fileList : []};
			var record = {queueId : DzmFrame.Uploader.S4(),name : name,size : null,type : null,filePath : filePath};
			jsonData.fileList.push(record);
			return JSON.stringify(jsonData);
		}
		uploader.S4 = function(){
			return (((1 + Math.random()) * 0x10000) | 0).toString(16).substring(1);
		}
		uploader.download = function(fileName,filePath){
			var url = ctx + '/fileUpload/download?fileName=' + fileName + '&path=' + filePath;
			window.open(url);
		}
		return uploader;
	}();
