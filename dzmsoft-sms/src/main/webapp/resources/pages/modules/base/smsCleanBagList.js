...
function initActions() {
	$('#searchFrom_find').on('click', function() {
		find();
	});
	$('#searchFrom_reset').on('click', function() {
		reset();
	});
	$('.add').linkbutton({
		'onClick' : function() {
...
function edit() {
	...
	dForm = $('#dlg').dialog({
		...
		height : 600,
		modal : true,
		href : ctx + '/smsCleanBag/edit/' + row.id,
		buttons : [ {
			text : '提交',
			handler : function() {
				submit();
...
function view() {
	...
	dForm = $('#dlg').dialog({
		...
		height : 600,
		modal : true,
		href : ctx + '/smsCleanBag/view/' + row.id,
		buttons : [ {
			text : '返回',
			handler : function() {
				dForm.panel('close');
...
function remove() {
	...
	parent.$.messager.confirm('提示', '删除后无法恢复您确定要删除？', function(data) {
		if (data) {
			$.ajax({
				type : 'get',
				url : ctx + "/smsCleanBag/remove/" + row.id,
...
function enable() {
	...
	parent.$.messager.confirm('提示', '您确定要启用吗？', function(data) {
		if (data) {
			$.ajax({
				type : 'put',
				url : ctx + "/smsCleanBag/enable/" + row.id,
				success : function(data) {
					if (data.success) {
						$.messager.alert('系统提示', data.msg, 'info', function() {
							find();
...
function disable() {
	...
	parent.$.messager.confirm('提示', '您确定要禁用？', function(data) {
		if (data) {
			$.ajax({
				type : 'put',
				url : ctx + "/smsCleanBag/disable/" + row.id,
				success : function(data) {
					if (data.success) {
						$.messager.alert('系统提示', data.msg, 'info', function() {
							find();
			...
			'submit',
			{
				onSubmit : function() {
					var isValid = $(this).form('enableValidation').form(
							'validate');
					if (isValid) {
						var datasSmsCleanBagLine = $('#SmsCleanBagLineGrid')
								.datagrid('getData');
						var datasJsonSmsCleanBagLine = JSON
								.stringify(datasSmsCleanBagLine.rows)
						$('#datasSmsCleanBagLine')
								.val(datasJsonSmsCleanBagLine);
						$.messager.progress();
					}
