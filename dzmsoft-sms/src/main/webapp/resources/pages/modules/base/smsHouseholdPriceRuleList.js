...
function createDg(){
	dgList=$('#dg').datagrid({    
	method: "post",url:ctx+'/smsHouseholdPriceRule/find', idField : 'id',singleSelect:true,
 ...
 */
function add(){
		var url = ctx+'/smsHouseholdPriceRule/add';
	window.top.addTab('新增家装保洁计价规则',url);
	}
/**
 * 编辑
 */
function edit(){
	...
	if(rowIsNull(row)) return;
		var url = ctx+'/smsHouseholdPriceRule/edit/'+row.id;
	window.top.addTab('编辑家装保洁计价规则',url);
	}
/**
 * 删除
 */
function remove(){
	...
	parent.$.messager.confirm('提示', '删除后无法恢复您确定要删除？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:ctx+"/smsHouseholdPriceRule/remove/"+row.id,
...
function enable(){
	...
	parent.$.messager.confirm('提示', '您确定要启用吗？', function(data){
		if (data){
			$.ajax({
				type:'put',
				url:ctx+"/smsHouseholdPriceRule/enable/"+row.id,
				success: function(data){
					if (data.success){
						$.messager.alert('系统提示',data.msg,'info',function(){
							find();
...
function disable(){
	...
	parent.$.messager.confirm('提示', '您确定要禁用？', function(data){
		if (data){
			$.ajax({
				type:'put',
				url:ctx+"/smsHouseholdPriceRule/disable/"+row.id,
				success: function(data){
					if (data.success){
						$.messager.alert('系统提示',data.msg,'info',function(){
							find();
