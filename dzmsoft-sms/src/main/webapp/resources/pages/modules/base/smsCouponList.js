...
function initActions(){
	...
	DateRange.initDateBox($('#effectiveDate_start'), $('#effectiveDate_end'));
	DateRange.initDateBox($('#expirationDate_start'), $('#expirationDate_end'));
}
/**
 * 查询
 */
...
function reset(){
	...
	DateRange.resetDateBox($('#effectiveDate_start'), $('#effectiveDate_end'));
	DateRange.resetDateBox($('#expirationDate_start'), $('#expirationDate_end'));
	find();
}
/*
 * 创建表格
...
function createDg(){
	dgList=$('#dg').datagrid({    
	method: "post",url:ctx+'/smsCoupon/find', idField : 'id',singleSelect:true,
    ...
    columns:[[    
    	    	    		    			{field:'id',title:'ID',hidden:true}
    		    	    		    			,{field:'faceValue',title:'面值',sortable:true,width:100,align:'left',halign:'right' }
    		    	    		    			...
    		    	    		    			,{field:'description',title:'优惠券描述',sortable:true,width:100,align:'left',halign:'center' }
    		    	    		    			,{field:'status',title:'状态',sortable:true,width:100,align:'left',halign:'center',formatter:fmtStatus }
    		    	    ]],
    sortName:'faceValue',sortOrder:'asc',
    ...
    onClickRow:function(index,row){
    	...
    	case DATA_STATUS.ENABLE: 
    		...
    		break;
    	case DATA_STATUS.DISABLED:case DATA_STATUS.INIT:
...
function edit(){
	...
	dForm=$('#dlg').dialog({title:'编辑',width: 380, height: 380,modal:true
		,href:ctx+'/smsCoupon/edit/'+row.id,
		buttons:[
			{text:'提交',handler:function(){submit();}}
			,{text:'返回',handler:function(){dForm.panel('close');}}
		]
...
function remove(){
	...
	parent.$.messager.confirm('提示', '删除后无法恢复您确定要删除？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:ctx+"/smsCoupon/remove/"+row.id,
...
function enable(){
	var row = dgList.datagrid('getSelected');
	if(rowIsNull(row)) return;
	parent.$.messager.confirm('提示', '您确定启用此票券吗？', function(data){
		if (data){
			$.ajax({
				type:'put',
				url:ctx+"/smsCoupon/enable/"+row.id,
				success: function(data){
					if (data.success){
						$.messager.alert('系统提示',data.msg,'info',function(){
							find();
...
function disable(){
	var row = dgList.datagrid('getSelected');
	if(rowIsNull(row)) return;
	parent.$.messager.confirm('提示', '您确定禁用此票券吗？', function(data){
		if (data){
			$.ajax({
				type:'put',
				url:ctx+"/smsCoupon/disable/"+row.id,
				success: function(data){
					if (data.success){
						$.messager.alert('系统提示',data.msg,'info',function(){
							find();
