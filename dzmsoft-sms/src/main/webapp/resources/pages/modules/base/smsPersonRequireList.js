...
function createDg(){
	dgList=$('#dg').datagrid({    
	method: "post",url:ctx+'/smsPersonRequire/find', idField : 'id',singleSelect:true,
    ...
    columns:[[    
    	    	    		    			{field:'id',title:'ID',hidden:true}
    		    	    		    			...
    		    	    		    			, {
    		    	    		    				...
    		    	    		    				align : 'left',
    		    	    		    				halign : 'center',
    		    	    		    				formatter : DzmFrame.EasyUI.fmtYyyyMMdd
    		    	    		    			}, {
 ...
 */
function add(){
	dForm=$('#dlg').dialog({title:'新增',width: 750, height: 600,modal:true
		,href:ctx+'/smsPersonRequire/add',
		buttons:[
...
function edit(){
	...
	dForm=$('#dlg').dialog({title:'编辑',width: 750, height: 600,modal:true
		,href:ctx+'/smsPersonRequire/edit/'+row.id,
		buttons:[
			{text:'提交',handler:function(){submit();}}
			,{text:'返回',handler:function(){dForm.panel('close');}}
		]
...
function view(){
	var row = dgList.datagrid('getSelected');
	if(rowIsNull(row)) return;
	dForm=$('#dlg').dialog({title:'查看',width: 750, height: 600,modal:true
		,href:ctx+'/smsPersonRequire/view/'+row.id,
		buttons:[
			{text:'返回',handler:function(){dForm.panel('close');}}
		]
	});
...
function remove(){
	...
	parent.$.messager.confirm('提示', '删除后无法恢复您确定要删除？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:ctx+"/smsPersonRequire/remove/"+row.id,
...
function enable(){
	...
	parent.$.messager.confirm('提示', '您确定要启用吗？', function(data){
		if (data){
			$.ajax({
				type:'put',
				url:ctx+"/smsPersonRequire/enable/"+row.id,
				success: function(data){
					if (data.success){
						$.messager.alert('系统提示',data.msg,'info',function(){
							find();
...
function disable(){
	...
	parent.$.messager.confirm('提示', '您确定要禁用？', function(data){
		if (data){
			$.ajax({
				type:'put',
				url:ctx+"/smsPersonRequire/disable/"+row.id,
				success: function(data){
					if (data.success){
						$.messager.alert('系统提示',data.msg,'info',function(){
							find();
...
function submit(){
	$('#mainform').form('submit',{
		onSubmit:function(){
			...
			if (isValid){
																																																																												var datasSmsPersonRequireLine = $('#SmsPersonRequireLineGrid').datagrid('getData');
										var datasJsonSmsPersonRequireLine = JSON.stringify(datasSmsPersonRequireLine.rows)
					$('#datasSmsPersonRequireLine').val(datasJsonSmsPersonRequireLine);
								$.messager.progress();
			}
