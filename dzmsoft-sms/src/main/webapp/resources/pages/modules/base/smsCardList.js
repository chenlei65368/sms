...
function initActions(){
	...
	DateRange.initDateBox($('#expierationDate_start'), $('#expierationDate_end'));
}
/**
 * 查询
 */
...
function reset(){
	...
	DateRange.resetDateBox($('#expierationDate_start'), $('#expierationDate_end'));
	find();
}
/*
 * 创建表格
...
function createDg(){
	dgList=$('#dg').datagrid({    
	method: "post",url:ctx+'/smsCard/find', idField : 'id',singleSelect:true,
...
function enable(){
	var row = dgList.datagrid('getSelected');
	if(rowIsNull(row)) return;
	parent.$.messager.confirm('提示', '您确定启用此卡片吗？', function(data){
		if (data){
			$.ajax({
				type:'put',
				url:ctx+"/smsCard/enable/"+row.id,
				success: function(data){
					if (data.success){
						$.messager.alert('系统提示',data.msg,'info',function(){
							find();
...
function disable(){
	var row = dgList.datagrid('getSelected');
	if(rowIsNull(row)) return;
	parent.$.messager.confirm('提示', '您确定禁用此卡片吗？', function(data){
		if (data){
			$.ajax({
				type:'put',
				url:ctx+"/smsCard/disable/"+row.id,
				success: function(data){
					if (data.success){
						$.messager.alert('系统提示',data.msg,'info',function(){
							find();
