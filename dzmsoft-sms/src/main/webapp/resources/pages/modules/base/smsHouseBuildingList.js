...
function createDg(){
	dgList=$('#dg').datagrid({    
	method: "post",url:ctx+'/smsHouseBuilding/find', idField : 'id',singleSelect:true,
    ...
    columns:[[    
    	    	    		    			{field:'id',title:'楼栋编号',hidden:true}
    		    	    		    			    			    			    			    			,{field:'name',title:'楼栋名称',sortable:true,width:100,align:"left",halign:'center' }
    		    	    		    			    			    			    			...
    		    	    		    			    			    			    			,{field:'housingId',title:'房源',sortable:true,width:100,align:"left",halign:'center' }
    		    	    ]],
...
function edit(){
	...
	if(rowIsNull(row)) return;
		...
		,href:ctx+'/smsHouseBuilding/edit/'+row.id,
		buttons:[
			{text:'提交',handler:function(){submit();}}
			,{text:'返回',handler:function(){dForm.panel('close');}}
		]
...
function enable(){
	...
	parent.$.messager.confirm('提示', '您确定要启用吗？', function(data){
		if (data){
			$.ajax({
				type:'put',
				url:ctx+"/smsHouseBuilding/enable/"+row.id,
				success: function(data){
					if (data.success){
						$.messager.alert('系统提示',data.msg,'info',function(){
							find();
...
function disable(){
	...
	parent.$.messager.confirm('提示', '您确定要禁用？', function(data){
		if (data){
			$.ajax({
				type:'put',
				url:ctx+"/smsHouseBuilding/disable/"+row.id,
				success: function(data){
					if (data.success){
						$.messager.alert('系统提示',data.msg,'info',function(){
							find();
