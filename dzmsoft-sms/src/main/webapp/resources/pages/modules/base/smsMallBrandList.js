...
function createDg(){
    ...
    columns:[[    
    	    	    		    			{field:'id',title:'品牌编号',sortable:true,width:100,align:'left',halign:'center'}
    		    	    		    			,{field:'name',title:'品牌名称',sortable:true,width:100,align:'left',halign:'center' }
    		    	    		    			,{field:'category',title:'商品类别',sortable:true,width:100,align:'left',halign:'center' }
...
function edit(){
	...
	dForm=$('#dlg').dialog({title:'编辑',width: 380, height: 380,modal:true
		,href:ctx+'/smsMallBrand/edit/'+row.id,
		buttons:[
			{text:'提交',handler:function(){submit();}}
			,{text:'返回',handler:function(){dForm.panel('close');}}
		]
...
function remove(){
	...
	parent.$.messager.confirm('提示', '删除后无法恢复您确定要删除？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:ctx+"/smsMallBrand/remove/"+row.id,
