...
function reloadTabGrid(parent,current, _callback){
       if ($("#mainTabs").tabs('exists', parent)) {
            $('#mainTabs').tabs('select' , parent);
            $('#mainTabs').tabs('close' , current);
            _callback();
            window.top.reload_oes_paper.call();
      }
}
/**
 * 新增页签
